...
def _render_market_overview():
    ...
    styled = fmt_df.style.map(_color_pct, subset=["1h %","24h %","7d %"])
...
def _render_reddit_crypto():
    ...
    styled = df.style.map(_rc_color, subset=["Rank Chg"])
...
def _render_futures():
    ...
    styled = fmt_df[show_cols].style.map(_fr_color, subset=["Funding Rate"] if "Funding Rate" in show_cols else [])
    ...
    if st.button("📡 Load DeFi Data", key="defi_load", type="primary") or \
       st.session_state.get("defi_loaded"):
        ...
        with tab_all:
            ...
            styled = show.style.map(_pct_color, subset=["1d %","7d %"])
            st.dataframe(styled, use_container_width=True, hide_index=True, height=400)
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 7 — TRADE (real order execution)
# ══════════════════════════════════════════════════════════════════════════════

def _get_or_create_crypto_trading_portfolio(db, tenant_id: str):
    """
    A dedicated portfolio for real crypto order execution -- separate
    from the "Crypto Tracker" portfolio the manual holdings widget
    above uses (modules.crypto.portfolio_sync). That widget deletes
    any position not present in its own session-state list on every
    render (see sync_crypto_holdings_to_portfolio's "Remove positions
    for symbols no longer held" step) -- sharing a portfolio would
    mean a real, trade-generated position could be silently wiped out
    just by visiting the Portfolio Tracker tab without having also
    manually re-added that same symbol there.
    """
    import uuid
    ...
    portfolio = (
        db.query(Portfolio)
        .filter(Portfolio.tenant_id == tenant_id, Portfolio.name == name)
        .first()
    )
    if portfolio:
        return portfolio
    ...
    portfolio = Portfolio(
        ...
        tenant_id=tenant_id,
        name=name,
        description="Real crypto order execution -- paper fills by default, live via a connected exchange once configured.",
        base_currency="USD",
...
def _render_crypto_trading(db=None, user=None):
    st.subheader("💱 Crypto Trading")
    st.caption(
        "Real order execution through the same trading pipeline stocks use -- "
        "simulated paper fills by default, or live via a connected exchange "
        "(Admin > Brokers). Distinct from the Portfolio Tracker tab above, "
        "which is for manually recording holdings you hold elsewhere, not "
        "for placing real orders."
    )

    if db is None or user is None:
        st.info("Trading requires a signed-in session.")
        return
    ...
    if not tenant_id:
        st.warning("No tenant context available -- trading is disabled.")
        return

    portfolio = _get_or_create_crypto_trading_portfolio(db, tenant_id)
    st.caption(f"Trading portfolio: **{portfolio.name}** ({portfolio.base_currency})")

    from api.services.crypto_orders_api_service import CryptoOrdersAPIService
    from api.services.orders_api_service import OrdersAPIService

    crypto_orders = CryptoOrdersAPIService(db)
    orders_service = OrdersAPIService(db)

    with st.form("crypto_trade_ticket"):
        t1, t2, t3, t4 = st.columns(4)

        symbol = t1.text_input(
            "Pair", value="BTC/USDT",
            help="ccxt-unified pair format, e.g. BTC/USDT -- not the exchange-specific BTCUSDT.",
        ).upper().strip()

        side = t2.selectbox("Side", ["buy", "sell"])

        qty = t3.number_input(
            "Quantity", min_value=0.0, step=0.001, format="%.6f", value=0.01,
            help="Amount of the base currency, e.g. 0.01 for 0.01 BTC.",
        )

        order_type = t4.selectbox("Order Type", ["market", "limit"])

        limit_price = None
        if order_type == "limit":
            limit_price = st.number_input("Limit Price", min_value=0.0, step=0.01, value=0.0)
        ...
        if submitted:
            if qty <= 0:
                ...
                st.error(f"'{symbol}' isn't a valid pair symbol. Use the ccxt-unified format, e.g. 'BTC/USDT'.")
            else:
                result = crypto_orders.create_order(
                    tenant_id=tenant_id,
                    user_id=user_id,
                ...
                if result is None:
                    st.error("Trading portfolio not found.")
                elif result.get("success"):
                    price_note = f"@ ${result['filled_price']:,.2f}" if result.get("filled_price") else "(pending)"
                    st.success(
                        f"Order {result.get('status')}: {side} {qty:g} {symbol} {price_note} "
                        f"(broker: {result.get('broker')})"
                    )
                    st.rerun()
                else:
                    st.error(f"Order rejected: {result.get('message')}")

    st.divider()

    st.markdown("#### Open Positions")

    positions = crypto_orders.get_positions(tenant_id=tenant_id)

    if positions["positions"]:
        st.dataframe(
            pd.DataFrame(positions["positions"]),
            use_container_width=True,
            hide_index=True,
        )
    else:
        st.info("No open crypto positions yet.")

    st.markdown("#### Recent Orders")

    from models.trading import TradeOrder, Portfolio as PortfolioModel

    orders = (
        db.query(TradeOrder)
        .join(PortfolioModel, PortfolioModel.id == TradeOrder.portfolio_id)
        .filter(PortfolioModel.tenant_id == tenant_id)
        .filter(TradeOrder.symbol.like("%/%"))
    ...
    _PENDING_STATUSES = ("pending", "new", "accepted", "open")

    header = st.columns([2, 1, 1, 1, 1, 1])
    for col, label in zip(header, ["Symbol", "Side", "Qty", "Status", "Fill Price", ""]):
        col.markdown(f"**{label}**")

    for o in orders:
        cols = st.columns([2, 1, 1, 1, 1, 1])
        cols[0].write(o.symbol)
        cols[1].write(o.side)
        cols[2].write(f"{o.qty:g}")
        cols[3].write(o.status)
        cols[4].write(f"${o.avg_fill_price:,.2f}" if o.avg_fill_price else "—")

        if str(o.status).lower() in _PENDING_STATUSES:
            if cols[5].button("Cancel", key=f"cancel_crypto_order_{o.id}"):
                cancel_result = orders_service.cancel_order(tenant_id=tenant_id, order_id=o.id)
                if cancel_result and cancel_result.get("cancelled"):
                    st.success("Order cancelled.")
                    st.rerun()
                else:
                    st.warning((cancel_result or {}).get("reason") or "Could not cancel this order.")