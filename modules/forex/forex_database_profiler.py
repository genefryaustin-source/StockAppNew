...
class ForexDatabaseProfiler:
 ...
 def end_query(self,t,rows=None,success=True,error=None):
  a=self._a.pop(t,None)
  if not a:
   return None
  ms=(time.perf_counter()-a["st"])*1000
  q="WRITE" if a["sql"].strip().upper().startswith(("INSERT","UPDATE","DELETE","CREATE","ALTER","DROP","MERGE")) else "READ"
  r=QueryRecord(module=a["module"],function=a["function"],sql=a["sql"],duration_ms=round(ms,3),rows=rows,session_id=a["sid"],connection_id=a["cid"],success=success,error=error,query_type=q)
  self._h.append(r)
  self.s["queries"]+=1
  self.s["total_ms"]+=r.duration_ms
  self.s["max_ms"]=max(self.s["max_ms"],r.duration_ms)
  self.s["reads" if q=="READ" else "writes"]+=1
  self.s["slow_queries"]+=1 if r.duration_ms>=SLOW_QUERY_MS else 0
  self.s["failures"]+=0 if success else 1
  return r
