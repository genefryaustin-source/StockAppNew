...
def render_forex_control_center(  db=None,
    user=None,) -> None:
    st.title("Forex Control Center")
    action = st.radio("Control Action", ["run_cycle", "schedule", "diagnostics", "heal", "optimize"], horizontal=True, key="fx_control_action")
    pairs = st.multiselect("Control Pairs", default_pairs(), default=default_pairs()[:3], key="fx_control_pairs")
    if st.button("Execute Control Action", key="fx_control_execute"):
        kwargs = {}
        if action in {"run_cycle", "schedule"}:
            kwargs["pairs"] = pairs
        st.json(ForexControlPlane().command(action, **kwargs))
    _render_snapshot("Control Plane Snapshot")
