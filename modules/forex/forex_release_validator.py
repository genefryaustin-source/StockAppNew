from __future__ import annotations
from datetime import datetime, timezone
from typing import Any, Dict

def _utc_now():
    return datetime.now(timezone.utc).replace(microsecond=0).isoformat()

def _safe_import(path,name):
    m=__import__(path,fromlist=[name]); return getattr(m,name)

class ForexReleaseValidator:
    def validate(self)->Dict[str,Any]:
        Center=_safe_import("modules.forex.forex_validation_center","ForexValidationCenter")
        payload=Center().run_full_validation(include_stress=True,stress_jobs=1000)
        card=payload.get("scorecard",{})
        ready=card.get("certified",False) or (card.get("score",0)>=95 and payload.get("failed",1)==0)
        return {"status":"ready" if ready else "blocked",
                "release_ready":ready,
                "validation":payload,
                "validated_at":_utc_now()}
