...
try:
    ...
    if role == "client":
        ...
        # at all for "client", and sidebar rendering crashed with
        # NameError: name 'NAV_GROUPS' is not defined on every single page
    ...
    # Always return this run's connection to the pool, even if the
    # script called st.stop() partway through (e.g. a permission
    # check) or raised -- without this, every single page render
    # leaves its connection checked out until Python's garbage
    # collector eventually notices, often long after Neon has
    # already killed it server-side for being idle.
    safe_rollback(db)
    safe_close(db)