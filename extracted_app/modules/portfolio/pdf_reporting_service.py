...
class PDFReportingService:
    ...
    def _nav_chart(self, nav_df):
        """
        NAV-over-time line chart. Returns a temp PNG file path (caller
        is responsible for deleting it, matching every other chart
        helper's contract via _save_chart), or None if there's nothing
        to plot -- generate_portfolio_report already treats None as
        "skip this image" rather than an error.
        """
        if nav_df is None or nav_df.empty or "NAV" not in nav_df.columns:
            return None

        fig, ax = plt.subplots(figsize=(8, 3.5))
        ax.plot(nav_df["Date"], nav_df["NAV"], linewidth=1.5, color="#1f4e79")
