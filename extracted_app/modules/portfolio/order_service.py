...
class OrderService:
    ...
    def _get_reference_price(self, symbol: str) -> float:
        ...
        try:
            ...
            try:
                from modules.crypto.pricing import is_crypto_pair_symbol, get_last_pricing_errors

                if is_crypto_pair_symbol(symbol):
                    detail_errors = get_last_pricing_errors(symbol)
                    if detail_errors:
                        raise ValueError(
                            f"Unable to get reference price for {symbol}: "
                            + "; ".join(detail_errors)
                        )
            except ImportError:
                pass
    ...
    def submit_order(
        ...
        try:
            ...
            order.position = None
            order.closed_trade = None
            order.planned_risk = None
            ...
            if float(broker_resp.filled_qty or 0) > 0 and broker_resp.avg_fill_price is not None:
                ...
                prior_planned_risk = (
                    float(position.planned_risk)
                    if getattr(position, "planned_risk", None) is not None
                    else None
                )
                ...
                # ---------------------------------------
                # planned_risk is the dollar amount the trader is willing
                # to lose on the *current* open position, derived from the
                # stop attached when it was opened or added to. It has to
                # survive on the position itself (not just this order)
                # because the trade that eventually closes the position
                # rarely carries its own stop price.
                if side_norm == "buy" and stop_price is not None and new_qty != 0:
                    order.planned_risk = abs(fill_price - float(stop_price)) * abs(new_qty)
                elif side_norm == "sell":
                    order.planned_risk = prior_planned_risk
                else:
                    order.planned_risk = prior_planned_risk

                if abs(new_qty) < 1e-9:
                    position.planned_risk = None
                elif side_norm == "buy" and stop_price is not None:
                    position.planned_risk = order.planned_risk
                else:
                    position.planned_risk = prior_planned_risk
                ...
                if side_norm == "sell" and prior_qty > 0:
                    ...
                    if closed_qty > 0:
                        ...
                        order.closed_trade = self.closed_trade_service.record_closed_trade(
                            portfolio_id=portfolio_id,
                            symbol=symbol_norm,
                            opened_at=opened_at,
                            closed_at=closed_at,
    ...
    def list_orders(
        ...
        """
        Return all orders for a portfolio after verifying
        the portfolio belongs to the authenticated tenant.
        """
