...
from modules.stocks.stock_trading_service import (
    StockTradingService,
    ExecutionResult,
)
...
def _format_execution_failure(result: ExecutionResult) -> str:
    ...
    qty = float(result.quantity or 0.0)

    return (
        f"{symbol} {side} {qty:g}: "
        f"{result.message or result.status or 'Execution failed'}"
    )
...
def _submit_stock_order(
    ...
    *,
    ...
    return trading_service.submit_order(
...
def _execute_stock_order_batch(
    trading_service: StockTradingService,
    *,
    portfolio_id,
    user_id,
    ...
    for _, row in orders_df.iterrows():
        result = _submit_stock_order(
            ...
            side=row["Side"],
            qty=row["Qty"],
            order_type=row.get("Order Type", "market"),
            tif=row.get("TIF", "day"),
            limit_price=row.get("Limit Price"),
            stop_price=row.get("Stop Price"),
            recommendation_id=row.get("Recommendation ID"),
        )
    ...
    return {
        ...
        "submitted": len(results) - len(failures),
        "failed": len(failures),
        "results": results,
        "failures": failures,
    }
...
def _render_batch_execution_result(
    batch_result: dict,
    *,
    success_message: str,
) -> bool:
    ...
    if failed:
        st.error(
            f"Execution partially failed: "
            f"{submitted} submitted, {failed} failed."
        )

        for result in batch_result.get("failures", []):
            st.write(f"- {_format_execution_failure(result)}")
        ...
        if _TRADE_PREVIEW_AVAILABLE:
            ...
            if result.success:
                st.success(
                    ...
                    f"{result.symbol} "
                    f"{result.side} "
                    f"{float(result.quantity or 0):g} "
                    f"| status={result.status} "
                    f"| broker_order_id={result.broker_order_id or 'N/A'}"
                )
                st.rerun()

            else:
                st.error(
                    f"Trade submission failed: "
                    f"{result.message or result.status}"
                )

    # ── Render pending trade preview (approve / reject gate) ──
    if (
            _TRADE_PREVIEW_AVAILABLE
            and st.session_state.get("pending_trade_preview")
    ):
        render_trade_preview(
            db_session,
            trading_service,
                        ...
                        if combined_rebalance_df.empty:
                            ...
                            try:
                                ...
                                if not valid:
                                    ...
                                    batch_result = _execute_stock_order_batch(
                                        trading_service,
                                        portfolio_id=portfolio_id,
                                        user_id=user_id,
                                        orders_df=combined_rebalance_df,
                                    )
                                    ...
                                    if batch_result["success"]:
                                        ...
                                        execution_complete = _render_batch_execution_result(
                                            batch_result,
                                            success_message="Combined rebalance executed.",
                                        )
                            ...
                            try:
                                ...
                                if not valid:
                                    ...
                                    batch_result = _execute_stock_order_batch(
                                        ...
                                        portfolio_id=portfolio_id,
                                        user_id=user_id,
                                        orders_df=optimized_rebalance_df,
                                    )
                                    ...
                                    if batch_result["success"]:
                                        ...
                                        execution_complete = _render_batch_execution_result(
                                            batch_result,
                                            success_message="Optimized rebalance executed.",
                                        )
        ...
        if st.button("Execute Rebalance Trades", key=f"execute_construction_{portfolio_id}"):
            try:
                batch_result = _execute_stock_order_batch(
                    ...
                    portfolio_id=portfolio_id,
                    user_id=user_id,
                    orders_df=rebalance_df,
                )

                execution_complete = _render_batch_execution_result(
                    batch_result,
                    success_message="Rebalance orders submitted.",
                )
            ...
            if alpha_rebalance_df.empty:
                ...
                try:
                    batch_result = _execute_stock_order_batch(
                        ...
                        portfolio_id=portfolio_id,
                        user_id=user_id,
                        orders_df=alpha_rebalance_df,
                    )

                    execution_complete = _render_batch_execution_result(
                        batch_result,
                        success_message="Alpha rebalance orders submitted.",
                    )
            ...
            if strategy_target_df.empty:
                ...
                if st.button("Execute Strategy", key=f"execute_strategy_{portfolio_id}"):
                    batch_result = _execute_stock_order_batch(
                        ...
                        portfolio_id=portfolio_id,
                        user_id=user_id,
                        orders_df=strategy_rebalance_df,
                    )

                    execution_complete = _render_batch_execution_result(
                        batch_result,
                        success_message="Strategy executed.",
                    )
                        ...
                        if ml_rebalance_df.empty:
                            ...
                            try:
                                ...
                                if not valid:
                                    ...
                                    execution_complete = _render_batch_execution_result(
                                        batch_result,
                                        success_message="ML rebalance executed.",
                                    )
            ...
            try:
                ...
                if not valid:
                    ...
                    if batch_result["success"]:
                        ...
                        execution_complete = _render_batch_execution_result(
                            batch_result,
                            success_message="Approved event rebalance executed.",
                        )

                        if execution_complete:
                            st.session_state.pop(
                                f"approved_event_rebalance_{portfolio_id}",
                                None,
                            )
                            st.rerun()
                ...
                if exec_col2.button("Execute Approved Rebalance", key=f"execute_approved_rebalance_{portfolio_id}"):
                    try:
                        ...
                        if not valid:
                            ...
                            execution_complete = _render_batch_execution_result(
                                batch_result,
                                success_message="Approved rebalance executed.",
                            )

                            if execution_complete:
                                st.session_state.pop(
                                    f"approved_rebalance_{portfolio_id}",
                                    None,
                                )
                                st.rerun()
