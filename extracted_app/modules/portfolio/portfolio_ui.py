...
def render_portfolio_ui(db_session, user, market_data_service):
    ...
    # 📉 EXECUTION ANALYTICS
    # =====================================================
    # Consolidates the institutional stock execution analytics stack:
    # events/audit, trade replay, attribution, AI review, compliance, and
    # the quality/broker/cost intelligence layer built on top of them.
    # Distinct from the "🧠 Intelligence" workspace above, which is about
    # portfolio-level fundamental scoring (composite/quality/growth/
    # momentum), not trade execution.
    if workspace == "📉 Execution Analytics":

        st.subheader("📉 Execution Analytics")
        st.caption("Institutional Stock Execution Analytics Stack")
        ...
        with exec_tabs[0]:
            try:
                from modules.stocks.stock_execution_dashboard import (
                    render_stock_execution_dashboard,
                )
                render_stock_execution_dashboard(
                    db_session,
                    portfolio_id=portfolio_id,
                ...
                st.error(f"Execution Events dashboard unavailable: {e}")

        with exec_tabs[1]:
            try:
                from modules.stocks.stock_trade_replay_dashboard import (
                    render_stock_trade_replay_dashboard,
                )
                # This dashboard has no portfolio_id parameter -- it
                # currently shows recent execution events across every
                # portfolio, not scoped to the one selected above.
                render_stock_trade_replay_dashboard(db_session)
            except Exception as e:
                st.error(f"Trade Replay dashboard unavailable: {e}")

        with exec_tabs[2]:
            try:
                from modules.stocks.stock_trade_attribution_dashboard import (
                    render_stock_trade_attribution_dashboard,
                )
                render_stock_trade_attribution_dashboard(
                    db_session,
                    portfolio_id=portfolio_id,
                ...
                st.error(f"Attribution dashboard unavailable: {e}")

        with exec_tabs[3]:
            try:
                from modules.stocks.stock_ai_trade_review_dashboard import (
                    render_stock_ai_trade_review_dashboard,
                )
                render_stock_ai_trade_review_dashboard(
                    db_session,
                    portfolio_id=portfolio_id,
                ...
                st.error(f"AI Review dashboard unavailable: {e}")

        with exec_tabs[4]:
            try:
                from modules.stocks.stock_compliance_dashboard import (
                    render_stock_compliance_dashboard,
                )
                render_stock_compliance_dashboard(
                    db_session,
                    portfolio_id=portfolio_id,
                ...
                st.error(f"Compliance dashboard unavailable: {e}")

        with exec_tabs[5]:
            try:
                from modules.stocks.stock_execution_intelligence_dashboard import (
                    render_stock_execution_intelligence_dashboard,
                )
                render_stock_execution_intelligence_dashboard(
                    db_session,
                    portfolio_id=portfolio_id,
                ...
                st.error(f"Execution Intelligence dashboard unavailable: {e}")