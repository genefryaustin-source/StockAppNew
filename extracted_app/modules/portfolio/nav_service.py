...
class NavService:
    ...
    def _filter_to_period(self, df, period_norm: str) -> pd.DataFrame:
        """
        Trims a full stored-history DataFrame down to the requested
        period, so the fast DB-cached path in _safe_get_price_history
        returns the same date range the live fetch would have --
        without this, "period=1mo" would silently return the entire
        stored history instead of just the last month of it.
        """
        from datetime import datetime, timedelta, UTC

        period_days = {
            "1d": 1, "5d": 5, "1mo": 30, "3mo": 90, "6mo": 182, "1y": 365, "2y": 730,
        }.get(period_norm)

        if period_days is None or "Date" not in df.columns:
            return df
    ...
    def _safe_get_price_history(self, symbol, period="6mo"):
        ...
        try:
            ...
            df = load_price_history(self.db, symbol_norm)

            # load_price_history returns "Date" as the index, not a
            # column (it calls df.set_index("Date", inplace=True)
            # internally) -- reset it back to a column here, matching
            # the shape the rest of this pipeline (and the live fetch
            # this is standing in for) expects.
            if df is not None and not df.empty:
                df = df.reset_index()
                df = self._filter_to_period(df, period_norm)
