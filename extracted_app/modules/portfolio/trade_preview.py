...
def _execute_trade(db_session, trading_service, params: dict):
    """Execute the order and clear the pending state."""

    try:
        result = trading_service.submit_order(**params)

        if result.success:

            st.session_state.pop(
                "pending_trade_preview",
                None,
            )

            st.session_state.pop(
                "pending_trade_params",
                None,
            )
            ...
            st.success(
                ...
                f"| Broker Order: {result.broker_order_id or 'N/A'}"
            )
