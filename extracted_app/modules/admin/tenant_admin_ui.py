...
def render_tenant_admin_panel(db, user):
    ...
    if tab_tenants:
        with tab_tenants:
            ...
            new_tenant_api_access = st.checkbox(
                "Grant external platform API access",
                value=False,
                key="tenant_ui_new_tenant_api_access",
                help=(
                    "Lets this tenant create and use API keys to call this "
                    "platform's own REST API (/api/v1/...) from external "
                    "systems. Off by default -- a new tenant has no external "
                    "API access until explicitly granted here or later from "
                    "the tenant's management panel below."
                ),
            )
            ...
            if st.button(
                ...
                if not new_tenant_name or not new_tenant_name.strip():
                    ...
                    new_id = service.create_tenant(
                        ...
                        platform_api_access_enabled=new_tenant_api_access,
                    )
                    if new_id is None:
                        st.error("Failed to create tenant due to a database error.")
                    else:
                        st.success(
            ...
            if not tenants:
                ...
                for tenant in tenants:
                    with st.expander(
                        ...
                        f"{tenant['name']}  "
                        f"(ID: {tenant['id'][:8]}...)  "
                        f"{'🌐 API access' if tenant['platform_api_access_enabled'] else '🚫 No API access'}"
                    ):
                        ...
                        if tenant["is_active"]:
                            if a1.button(
                                ...
                                if service.deactivate_tenant(tenant["id"]):
                                    st.rerun()
                                else:
                            ...
                            if a1.button(
                                ...
                                if service.activate_tenant(tenant["id"]):
                                    st.rerun()
                                else:
                        ...
                        api_col1.caption(
                            "🌐 External platform API access is currently "
                            + ("**enabled**" if tenant["platform_api_access_enabled"] else "**disabled**")
                            + " for this tenant."
                        )
                        if tenant["platform_api_access_enabled"]:
                            if api_col2.button(
                                "Revoke API access",
                                key=f"tenant_ui_revoke_api_{tenant['id']}",
                            ):
                                if service.set_platform_api_access(tenant["id"], False):
                                    st.success(
                                        "External API access revoked. Existing API "
                                        "keys for this tenant will stop working "
                                        "immediately."
                                    )
                                    st.rerun()
                                else:
                                    st.error("Failed to revoke API access.")
                        else:
                            if api_col2.button(
                                "Grant API access",
                                key=f"tenant_ui_grant_api_{tenant['id']}",
                            ):
                                if service.set_platform_api_access(tenant["id"], True):
                                    st.success(
                                        "External API access granted. This tenant "
                                        "can now create API keys from the 🌐 "
                                        "External API Access tab."
                                    )
                                    st.rerun()
                                else:
                                    st.error("Failed to grant API access.")
