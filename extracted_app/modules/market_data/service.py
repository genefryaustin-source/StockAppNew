...
def get_latest_price_map(symbols):
    ...
    from modules.crypto.pricing import is_crypto_pair_symbol, get_latest_crypto_price_map

    crypto_symbols = [s for s in symbols if is_crypto_pair_symbol(s)]
    stock_symbols = [s for s in symbols if s not in crypto_symbols]

    out = {}

    if crypto_symbols:
        # ccxt-style pair symbols (e.g. "BTC/USDT") -- these would
        # otherwise be silently dropped by _valid_base_symbol below
        # (a stock-ticker validator that rejects anything containing
        # "/") before ever reaching a stock data provider.
        out.update(get_latest_crypto_price_map(crypto_symbols))

    if stock_symbols:
        print("=" * 80)
        print("GET_LATEST_PRICE_MAP")
        print("COUNT:", len(stock_symbols))
        print("FIRST 20:", stock_symbols[:20])
        print("=" * 80)

        out.update(get_latest_prices(stock_symbols))
