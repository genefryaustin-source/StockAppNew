...
def get_positions_df(db, tenant_id: Optional[str] = None, portfolio_id: Optional[str] = None) -> pd.DataFrame:
    ...
    if portfolio_id is None:
        ...
        # Options positions live in the Options module's own
        # options_positions table for the same reason forex does (strike/
        # expiry/option_type fields the generic schema doesn't have), and
        # are likewise tenant-scoped rather than tied to a specific
        # equities-style Portfolio -- same tenant-wide-only scope as forex.
        from modules.risk_layer.options_bridge import get_options_positions_df
        options_df = get_options_positions_df(db, tenant_id)
        if not options_df.empty:
            df = pd.concat([df, options_df], ignore_index=True, sort=False)
