...
get_options_positions_df() is the positions counterpart to forex_bridge's
get_forex_positions_df(): the Options module persists its own open
positions in `options_positions` (modules.options.options_models),
separate from the generic PortfolioPosition table this Risk Layer reads
by default for equities/crypto -- options positions need fields (strike,
expiry, option_type) the generic schema doesn't have. This function reads
that table via the Options module's own get_positions() accessor (no
duplicate SQL, same principle forex_bridge follows) and normalizes it
into the shared positions_df shape, with extra Option Type / Strike /
Expiry / DTE columns populated only for options rows.
"""
...
def get_options_positions_df(db, tenant_id):
    """
    All persisted options positions for a tenant, normalized to the Risk
    Layer's standard positions_df columns plus Option Type / Strike /
    Expiry / DTE. Returns an empty DataFrame (not an error) if the
    Options module's tables aren't reachable or the tenant has no open
    positions -- this is read-only and must never break the rest of the
    Risk Layer snapshot.

    Reads the persisted `options_positions` snapshot, not a live broker
    call -- that snapshot is kept fresh by the Options module itself
    (modules.options.options_ui persists it whenever a user opens the
    Options Positions view). If that view hasn't been opened recently for
    this tenant, the snapshot may be stale or empty; this function has no
    way to know that and doesn't guess.
    """
    import pandas as pd
    ...
    try:
        ...
        rows = get_positions(db, tenant_id)
    except Exception:
        return pd.DataFrame()
    ...
    for r in rows:
        ...
        market_price = (market_value / multiplier) if multiplier else 0.0

        out_rows.append({
            "Portfolio ID": f"options:{tenant_id}",
            "Symbol": r.get("option_symbol") or r.get("underlying") or "",
            "Quantity": qty,
            "Avg Cost": float(r.get("avg_cost") or 0.0),
            "Market Price": market_price,
            "Market Value": market_value,
            "Unrealized P&L": float(r.get("unrealized_pnl") or 0.0),
            "Realized P&L": 0.0,
            "Option Type": r.get("option_type") or "",
