...
# Public alias -- modules.market.commodities_data (and any other future
# caller) reuses this proven, working Yahoo Finance fetch rather than
# duplicating the same raw HTTP call.
fetch_yahoo_history = _fetch_yahoo_history_raw
...
def _load_macro_snapshot() -> dict:
    ...
    with _TPE(max_workers=2) as _outer:
        fred_future = _outer.submit(_parallel_load_fred, fred_ids)
        yahoo_future = _outer.submit(_parallel_load_yahoo, yahoo_requests)
