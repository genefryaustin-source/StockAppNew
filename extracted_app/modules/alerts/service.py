...
class AlertService:
    ...
    def create_alert(
        ...
        """
        Create a single alert directly (as opposed to run_alert_checks,
        which derives alerts from analytics/price data). tenant_id and
        title are required -- both are NOT NULL columns on AlertEvent
        with no default. Returns the created AlertEvent, or None (not
        an exception) on a database error.
        """
        try:
            alert = AlertEvent(
                tenant_id=tenant_id,
