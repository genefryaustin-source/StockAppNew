...
def _inspector_cache(db: Session):
    """
    One SQLAlchemy Inspector + its table/column lookups, cached on the
    Session object itself for the life of this request -- every one of
    the 12 metric functions below calls _has_table/_has_column
    repeatedly, and re-inspecting the schema on every call is wasted
    work within a single request.
    """
    cache = getattr(db, "_executive_dashboard_schema_cache", None)
    if cache is None:
        ...
        cache = {"inspector": inspector, "tables": tables, "columns": {}}
        db._executive_dashboard_schema_cache = cache
...
def _has_table(db: Session, table: str) -> bool:
    ...
    return table in _inspector_cache(db)["tables"]
...
def _has_column(db: Session, table: str, column: str) -> bool:
    ...
    if table not in cache["columns"]:
        try:
            cols = cache["inspector"].get_columns(table)
            cache["columns"][table] = {c["name"] for c in cols}
        except Exception:
            cache["columns"][table] = set()

    return column in cache["columns"][table]
