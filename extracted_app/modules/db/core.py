...
def init_database():
    ...
    try:
        ...
        tenant_column_migrations = [
            ...
            ("platform_api_access_enabled", "BOOLEAN DEFAULT FALSE"),
            # stocks/options/forex default TRUE -- see the long comment
            # on these columns in modules/db/models.py for why (these
            # have been working, ungated features for every existing
            # tenant; a licensing gate defaulting them off would take
            # that away silently). crypto defaults FALSE: no trading
            # capability exists behind it yet at all.
            ("module_stocks_enabled", "BOOLEAN DEFAULT TRUE"),
            ("module_options_enabled", "BOOLEAN DEFAULT TRUE"),
            ("module_forex_enabled", "BOOLEAN DEFAULT TRUE"),
            ("module_crypto_enabled", "BOOLEAN DEFAULT FALSE"),
        ]
    ...
    # Users table migrations -- same approach and same reasoning as the
    # tenant migrations above (inspect() + plain ADD COLUMN, portable
    # across Postgres/SQLite). The only "users migrations" list that
    # previously existed in this function was entirely commented out
    # (dead code), so User.name (modules/db/models.py) would otherwise
    # never actually get created on any database where the users table
    # already existed before this column was added to the model.
    try:
        inspector = inspect(engine)
        ...
        for col_name, col_ddl in user_column_migrations:
            if col_name not in existing_user_cols:
                with engine.begin() as conn:
                    conn.execute(text(f"ALTER TABLE users ADD COLUMN {col_name} {col_ddl}"))
                print(f"USERS migration: added column '{col_name}'")

        print("USERS column migrations complete")
    except Exception as e:
        print("USERS column migrations skipped:", e)

    # Portfolios table migrations -- same approach and same reasoning
    # as the tenant/user migrations above. models.trading.Portfolio's
    # user_id column (which portfolio) is owned by within a tenant)
    # would otherwise never actually get created on any database where
    # the portfolios table already existed before this column was
    # added to the model.
    try:
        inspector = inspect(engine)
        existing_portfolio_cols = {c["name"] for c in inspector.get_columns("portfolios")}

        portfolio_column_migrations = [
            ("user_id", "VARCHAR(36)"),
        ]

        for col_name, col_ddl in portfolio_column_migrations:
            if col_name not in existing_portfolio_cols:
                with engine.begin() as conn:
                    conn.execute(text(f"ALTER TABLE portfolios ADD COLUMN {col_name} {col_ddl}"))
                print(f"PORTFOLIOS migration: added column '{col_name}'")

        print("PORTFOLIOS column migrations complete")
    except Exception as e:
        print("PORTFOLIOS column migrations skipped:", e)
