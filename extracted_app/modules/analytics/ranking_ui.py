...
def render_ai_rankings(db, user, price_data):
    ...
    regime_inputs = compute_market_regime_inputs(db)

    if regime_inputs is not None:
        market_regime = detect_market_regime(

            market_return_30d=regime_inputs["market_return_30d"],
            market_return_90d=regime_inputs["market_return_90d"],

            volatility_30d=regime_inputs["volatility_30d"],

            drawdown_90d=regime_inputs["drawdown_90d"],
        )
    else:
        # Not enough stored price history yet to compute this
        # honestly -- a neutral regime with 0.0 inputs is an explicit
        # "unknown/default", not a fabricated "flat market" reading
        # dressed up as a real computation.
        market_regime = detect_market_regime()
        st.info(
            "Not enough stored price history for the market benchmark yet -- "
            "showing a neutral default regime rather than a computed one."
        )
