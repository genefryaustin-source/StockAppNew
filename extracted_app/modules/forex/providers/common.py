...
# A single slow/unresponsive provider used to be able to block a render for
# up to this full timeout before failing over -- confirmed directly when
# frankfurter.dev returned a 520 error only after ~7.5s, and the client had
# no reason to give up any sooner since the ceiling was 20s. Providers that
# are actually up typically respond in well under a second in every log
# we've captured; anything still waiting past a few seconds is failing, not
# slow, so failing fast and letting the router's fallback chain take over
# is strictly better than patiently waiting out a dead provider.
DEFAULT_TIMEOUT = 5
...
def session() -> requests.Session:
    ...
    retry = Retry(
        total=3,
        backoff_factor=0.5,
        # 429 removed: the provider router already fails over to the next
        # provider and tracks rate-limit cooldowns itself. Retrying a 429
        # here just adds 3.5s of blocking sleep before giving up anyway --
        # better to fail fast and let the router move on immediately.
