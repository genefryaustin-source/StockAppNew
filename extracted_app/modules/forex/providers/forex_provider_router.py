...
DEFAULT_PROVIDER_MODULES: list[tuple[str, str]] = [
    ...
    # marketdata_fx has no corresponding module file -- every availability
    # check for every pair was silently attempting and failing this import,
    # with no console output to show it was happening.
    # ("marketdata_fx", "modules.forex.providers.marketdata_forex_provider"),
    # finnhub_fx has failed 100% of the time in every session log this
    # investigation has captured (429 or JSONDecodeError on an empty body) --
    # looks like an exhausted/invalid API key, not a transient issue.
    # ("finnhub_fx", "modules.forex.providers.finnhub_forex_provider"),
    # alpha_vantage_fx has returned "no usable rate" 100% of the time in
    # every session log this investigation has captured.
    ...
    # exchangerate_host has returned "no usable rate" 100% of the time in
    # every session log this investigation has captured, across multiple
    # separate restarts -- costing 800-950ms per pair before falling
    # through to a provider that actually works.
...
class ForexProviderRouter:
    ...
    def mark_failure(self, provider_name: str, error: Optional[str] = None, cooldown_seconds: int = 0) -> None:
        ...
        with self._lock:
            ...
            if is_auth_error(error):
                ...
                # Generic failures (bad JSON, "no usable rate", etc.) previously
                # got no cooldown at all, so a provider stuck returning garbage
                # was retried on every single quote/history request forever.
                provider.cooldown_until = utc_now() + timedelta(seconds=FAILURE_COOLDOWN_SECONDS)
    ...
    def get_history(
        ...
        *,
        ...
        Debug instrumentation added during this session's freeze investigation:
        this method previously had zero console output at all (unlike get_quote(),
        which prints PAIR/PROVIDER/LATENCY/RESULT for every attempt). The Live
        Chart calls this once per pair render via forex_history_service.py, and
        with the quote-side bottlenecks now fixed, remaining page-load time with
        no further console output is very likely hiding in here -- this is a
        fully serial loop over every ranked provider with no visibility into
        which one is slow or how many are being tried. These prints mirror the
        existing "FOREX ALPHA DEBUG"/"PARALLEL QUOTE DOWNLOAD" convention so the
        next real page load will show exactly what this call is doing instead
        of going silent.
        """

        _history_t0 = time.perf_counter()
        pair = normalize_pair(pair)
        print("=" * 80)
        print("FOREX HISTORY DEBUG | REQUEST")
        print("pair       :", pair)
        ...
        if not force_refresh:
            ...
            if cached:
                print("=" * 80)
                print("FOREX HISTORY DEBUG | CACHE HIT")
                print("pair  :", pair)
                print(f"elapsed_ms: {round((time.perf_counter() - _history_t0) * 1000.0, 2)}")
                print("=" * 80)
                return cached
        if not pair or "/" not in pair:
        ...
        print("=" * 80)
        print("FOREX HISTORY DEBUG | CACHE MISS -- trying providers")
        print("candidates:", [p.provider for p in ranked])
        print("=" * 80)
        ...
        for provider in ranked:
            ...
            if not callable(fn):
                ...
                print(f"FOREX HISTORY DEBUG | {provider.provider}: no history function available, skipping")
                continue

            started = time.perf_counter()
            try:
                print(f"FOREX HISTORY DEBUG | REQUEST -> {provider.provider} for {pair}")
                response = fn(
                    pair,
                    start_date=start_date,
                    end_date=end_date,
                ...
                print("=" * 80)
                print(f"HISTORY PROVIDER : {provider.provider}")
                print(f"LATENCY          : {latency_ms:.2f} ms")
                ...
                if rows:
                    self.mark_success(provider.provider, latency_ms=latency_ms)
                    print(f"FOREX HISTORY DEBUG | SUCCESS via {provider.provider} | total_elapsed_ms: {round((time.perf_counter() - _history_t0) * 1000.0, 2)}")
                    return {
                        "status": "OK",
                        "pair": pair,
        ...
        print("=" * 80)
        print("FOREX HISTORY DEBUG | ALL PROVIDERS EXHAUSTED")
        print("errors:", errors)
        print(f"total_elapsed_ms: {round((time.perf_counter() - _history_t0) * 1000.0, 2)}")
        print("=" * 80)
        return {
            "status": "ERROR",
            "pair": pair,
    ...
    def get_histories(
        ...
        print("=" * 80)
        print("PARALLEL HISTORY DOWNLOAD")
        print("Workers :", workers)
