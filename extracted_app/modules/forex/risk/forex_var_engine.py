...
class ForexVaREngine:
    ...
    def set_portfolio(
        ...
        for position in self.portfolio.positions:
            self.ensure_return_series(position.symbol, position.volatility)
    ...
    def ensure_return_series(
            self,
            symbol,
            annualized_volatility=None,
    ):
        """
        VaR/ES/Monte Carlo all read from self.return_series, which is only
        ever populated by explicit add_return_series() calls. A freshly
        added position with no return history silently produces $0 VaR
        everywhere, which looks like "no risk" rather than "no data" -
        actively misleading on a risk dashboard.

        If the caller has already supplied a real historical series for
        this symbol (e.g. via add_return_series() backed by actual market
        data), this is a no-op. Otherwise this generates a clearly-labeled
        synthetic daily return series (deterministic per symbol, so results
        are stable across calls in a session) so every calculation has
        something to work with today. Replace this with a real market-data
        historical series per symbol as soon as one is available - swap in
        add_return_series(symbol, real_returns) and it takes over.
        """

        if symbol in self.return_series:
            existing = self.return_series[symbol]
            if existing.returns:
                return existing

        real_series = self._fetch_real_return_series(symbol)

        if real_series is not None:
            self.return_series[symbol] = real_series
            self.clear_cache()
            return real_series

        daily_vol = safe_float(annualized_volatility, 0.0)

        if daily_vol <= 0:
            # No volatility supplied on the position either - fall back to
            # a reasonable default for a major FX pair (~9.5% annualized,
            # i.e. roughly 0.6% daily).
            daily_vol = 0.006

        rng = np.random.default_rng(
            abs(hash(symbol)) % (2 ** 32)
        )

        returns = rng.normal(
            loc=0.0,
            scale=daily_vol,
            size=self.configuration.lookback_days,
        ).tolist()
        ...
        series = HistoricalReturnSeries(
            ...
            metadata={
                "note": (
                    "Synthetically generated from an assumed/estimated "
                    "volatility, not real historical prices. Call "
                    "add_return_series() with real market data to replace it."
                ),
                "assumed_daily_volatility": daily_vol,
            },
        )

        self.return_series[symbol] = series
    ...
    def _fetch_real_return_series(self, symbol):
        """
        Best-effort attempt to source a real historical return series for
        `symbol` before falling back to the synthetic estimate above.

        Two sources are tried, cheapest first:

        1. Already-persisted history in the database (ForexHistoryService /
           ForexHistoryRepository), which is a fast, local, no-network read.
        2. A live pull through the Forex provider router (same router the
           price service uses), which hits real market-data providers and
           persists whatever it gets back for next time.

        Either step can legitimately be unavailable - no DB session, no
        network access, no API keys configured for the keyed providers, an
        offline/sandboxed environment, etc. All of that is treated as a
        normal, expected outcome (not an error): this method logs what it
        tried on the returned series' metadata when it succeeds, and simply
        ...
        try:
            ...
            self._record_return_source_attempt(
                symbol, "unavailable", f"forex_history_service import failed: {exc}"
            )
            return None
        ...
        try:
            ...
            self._record_return_source_attempt(
                symbol, "unavailable", f"forex_history_service init failed: {exc}"
            )
            return None

        # Step 1: whatever is already persisted locally - cheap, no network.
        try:
            history_service.ensure_tables()
            frame = history_service.load_history([symbol])
        except Exception as exc:
            frame = None
            self._record_return_source_attempt(
                symbol, "database_read_failed", str(exc)
            )

        series = self._return_series_from_frame(
            symbol, frame, min_observations, provider="database"
        )
        if series is not None:
            return series

        # Step 2: try a live provider pull. This can fail for entirely
        # normal reasons (no network egress to the provider, no API key
        # configured, provider outage) - all treated as "not available".
        try:
            frame = history_service.get_market_data(
                source="Live Providers",
                pairs=[symbol],
            )
        except Exception as exc:
            self._record_return_source_attempt(
                symbol, "live_fetch_failed", str(exc)
            )
            return None

        series = self._return_series_from_frame(
            symbol, frame, min_observations, provider="live_provider"
        )
        if series is not None:
            return series
        ...
        self._record_return_source_attempt(
            symbol, "no_data",
            "no persisted history and no live provider returned usable rows",
        )
        return None

    # ------------------------------------------------------------------

    def _return_series_from_frame(self, symbol, frame, min_observations, provider):
        ...
        try:
            ...
            def _norm(value):
                return str(value or "").upper().replace("/", "").replace("-", "").replace("_", "").replace(" ", "")

            target = _norm(symbol)

            if "pair" in working.columns:
                working = working[working["pair"].map(_norm) == target]
            elif "symbol" in working.columns:
                working = working[working["symbol"].map(_norm) == target]

            if "asof" in working.columns:
                working = working.sort_values("asof")

            closes = working["close"].astype(float).tolist() if "close" in working.columns else []
            dates = (
                working["asof"].astype(str).tolist()
                if "asof" in working.columns
                else []
            )
        except Exception:
        ...
        self._record_return_source_attempt(
            symbol, "success", f"{len(returns)} observations from {provider}"
        )
        ...
        return HistoricalReturnSeries(
            ...
            metadata={
                "note": "Real historical daily returns derived from persisted/live market close prices.",
                "provider": provider,
    ...
    def _record_return_source_attempt(self, symbol, outcome, detail):
        log = self.statistics.setdefault("return_series_source_log", [])
        log.append({
            "symbol": symbol,
            "outcome": outcome,
            "detail": detail,
    ...
    def add_return_series(
        ...
        if isinstance(series, HistoricalReturnSeries):
            returns = series.returns

        elif isinstance(series, pd.Series):
            returns = series.astype(float).tolist()

        elif isinstance(series, np.ndarray):
            returns = series.astype(float).reshape(-1).tolist()

        elif isinstance(series, pd.DataFrame):

            #
            # Accept a single numeric column.
            #
            if len(series.columns) != 1:
                raise ValueError(
                    f"{symbol}: DataFrame must contain exactly one return column."
                )
        ...
        self.return_series[symbol] = HistoricalReturnSeries(
            symbol=symbol,
            returns=returns,
        )
    ...
    def portfolio_return_vector(
        ...
        for position in self.portfolio.positions:
            ...
            print("=" * 80)
            print("HISTORICAL RETURN MATRIX")
            for i, row in enumerate(matrix):
                print(
                    i,
                    type(row),
                    len(row) if hasattr(row, "__len__") else "N/A",
                    type(row[0]) if hasattr(row, "__len__") and len(row) else None,
                )
            print("=" * 80)
        return np.array(
            returns,
    ...
    def historical_return_matrix(self) -> np.ndarray:
        ...
        for symbol in symbols:
            ...
            history = self.get_return_series(symbol)
            ...
            if isinstance(returns, pd.Series):
                s = returns.astype(float).reset_index(drop=True)
        ...
        #
        # Rows = assets, columns = observations for np.cov(rowvar=True)
        #
    ...
    def execute_scenario(
        ...
        # Scenarios are keyed by StressScenario.value, which is lowercase
        # ("rate_shock"), but callers (e.g. the dashboard's selectbox) pass
        # uppercase names like "RATE_SHOCK". Normalize so both work, and
        # also accept a StressScenario enum member directly.
        lookup_key = (
            scenario_name.value
            if isinstance(scenario_name, StressScenario)
            else str(scenario_name).strip().lower()
        )

        if lookup_key not in library:
            raise ValueError(
...
def get_forex_var_engine(
    ...
    if (
        ...
        # Every Streamlit rerun hands this a brand-new, short-lived DB
        # session (see app.py's new_db_session()), so comparing db identity
        # here rebuilt this engine -- wiping its return_series/
        # covariance_matrix/market_prices caches -- on every single render,
        # even though the underlying FX historical data barely changes
        # render to render. Reuse the existing engine (and its caches) for
        # the same tenant/user/portfolio; just point it at the current
        # request's live session.
        _VAR_ENGINE.db = db
