...
class ForexPortfolioOptimizer:
    ...
    def optimize(
        ...
        return {
            ...
            #
            # NOTE: this used to reference an undefined `runtime_source`
            # name (only `source` was ever assigned above), so every call
            # to optimize() raised a NameError right after the
            # "Alpha Source: ..." print -- on every code path, not just
            # "local". Whatever calls this (daily_briefing() ->
            # forex_strategy_lab -> ForexPortfolioOptimizer.optimize())
            # was swallowing or hanging on that exception with no further
            # console output, matching reports of the terminal freezing
            # immediately after "Alpha Source: local" with nothing printed
            # afterward.
            "runtime_source": source,

            "used_shared_runtime": source == "runtime",
        }
