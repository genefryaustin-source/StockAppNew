...
class ForexTradingDeskDashboard:
    ...
    def _build_live_portfolio_packet(self, **kwargs):
        ...
        # This call was unconditionally passing refresh=True, persist=True on
        # every single render -- unlike forex_terminal_dashboard.py's render(),
        # which throttles real refreshes to at most once every 15 seconds per
        # session. With no throttle here, EVERY Streamlit rerun of the Trading
        # Desk workspace (not just navigating to it, but any widget click
        # while on it) forced a brand new refresh_positions() live-provider
        # round trip plus a DB persist plus a full get_terminal_snapshot()
        # rebuild -- each costing several seconds per real console captures.
        # That explains the Trading Desk tab taking far longer to respond
        # than the Institutional Terminal. Mirror the same 15s throttle here;
        # the data is still real, just not needlessly re-fetched/persisted
        # multiple times within the same short window.
        do_refresh = True
        do_persist = True
        if st is not None:
            import time as _time
            now_ts = _time.time()
            last_refresh_ts = st.session_state.get("fx_trading_desk_last_refresh_ts", 0.0)
            if now_ts - last_refresh_ts >= 15.0:
                st.session_state["fx_trading_desk_last_refresh_ts"] = now_ts
                do_refresh = True
                do_persist = True
            else:
                do_refresh = False
        ...
        # ForexService.get_quotes() (forex_service.py) is a plain serial
        # dict comprehension -- {pair: self.get_quote(pair) for pair in
        # selected} -- calling get_quote() one pair at a time with zero
        # parallelism. Real console timestamps from a live capture showed
        # this fetching ~19 watchlist pairs one at a time, each pair taking
        # 1-9+ seconds (the aggregator tries its own provider waterfall per
        # pair), for a combined 70+ seconds on this single step -- the
        # dominant real cost behind the reported "almost 1 minute 30
        # seconds" Trading Desk load. It was invisible until now because
        # nothing printed elapsed time for this specific step (unlike every
        # other quote path in this codebase, which already fetches pairs in
        # parallel -- see "PARALLEL QUOTE DOWNLOAD" elsewhere). Fetch the
        # same pairs in parallel here too, preserving the exact same
        # ForexQuote objects/attributes the rows loop below expects.
        import concurrent.futures as _futures

        quotes: dict = {}

        if pairs:
            with _futures.ThreadPoolExecutor(max_workers=6) as _executor:
                _future_to_pair = {
                    _executor.submit(self.desk.forex_service.get_quote, _pair): _pair
                    for _pair in pairs
                }
                for _future in _futures.as_completed(_future_to_pair):
                    _pair = _future_to_pair[_future]
                    try:
                        _quote = _future.result()
                    except Exception:
                        continue
