"""
modules/forex/ui/forex_quant_research_workspace.py

Renders the "Quant Research" sub-tab: research rankings, top
opportunities, confidence distribution, alpha heatmap, factor
contribution, model diagnostics, and research timeline.

Fixed in this pass (no-fake-data audit): removed a hardcoded 3-row
fallback (EUR/USD/USD-CHF/AUD-USD with invented confidence, alpha,
momentum, carry, macro, and liquidity scores) that used to render whenever
the payload had no real research rows. Also removed fabricated defaults
for "Alpha Models" (was a fixed 7) and "Avg Confidence" (was a fixed 86%),
and the "Research Status" card now reflects whether real rows are actually
present instead of always reading "READY".
"""
...
def _extract_rows(payload):
    ...
    # No hardcoded fallback candidates: an empty list here honestly ranks
    # to "No rows available" instead of a fixed EUR/USD/USD-CHF/AUD-USD
    # sample with made-up momentum/carry/macro/liquidity scores.
    return _rank_rows(candidates)

def _cards(payload,rows):
    # No fabricated defaults: "models" and "Avg Confidence" previously fell
    # back to fixed 7 / 86% when the payload had nothing real to report --
    # they now honestly fall back to 0, and "Research Status" reflects
    # whether any real rows are actually present instead of always
    # reading "READY".
    stats=summarize_quant_rows(rows); models=extract_count(payload,["models","active_models"],0); validated=extract_count(payload,["validated","passed","valid_count"],0)
    avg=stats.get("confidence_mean",0) or extract_score(payload,["confidence","confidence_score","ai_confidence"],0)
    research_status = "READY" if rows else "NO_DATA"
    return [
        {"label":"Research Universe","value":len(rows),"caption":"FX opportunities","progress":min(len(rows)*6,100),"status":"ACTIVE" if rows else "WATCH","icon":"🌐"},
        {"label":"Signals Generated","value":len(rows),"caption":"Ranked signals","progress":min(len(rows)*8,100),"status":"READY" if rows else "WATCH","icon":"📡"},
        {"label":"Alpha Models","value":models,"caption":"Model stack","progress":min(models*12,100),"status":"READY" if models else "WATCH","icon":"🧠"},
        ...
        {"label":"Sharpe","value":f"{stats.get('sharpe',0):.2f}","caption":"Research distribution","progress":min(abs(float(stats.get('sharpe',0)))*25,100),"status":"ACTIVE" if rows else "WATCH","icon":"📈"},
        {"label":"Hit Rate","value":f"{stats.get('win_rate',0):.0f}%","caption":"Positive alpha share","progress":stats.get("win_rate",0),"status":"READY" if rows else "WATCH","icon":"🎯"},
        {"label":"Research Status","value":research_status,"caption":"Live rows" if rows else "No live rows available","progress":100 if rows else 0,"status":research_status,"icon":"🟢" if rows else "⚪"},
    ]
