"""
modules/forex/ui/forex_ai_workspace.py

Renders the "AI & Quant Platform" tab of the Institutional Terminal --
Executive summary, Quant Research, Factor Models, Regime, Alpha,
Signal Validation, Portfolio Optimizer, Strategy Lab, AI Committee, and
Enterprise Reports sub-tabs.

Fixed in this pass (no-fake-data audit): removed hardcoded fallback data
that used to render regardless of real conditions -- a 3-row EUR/USD /
USD-CHF / AUD-USD opportunity list, a RISK_OFF/78 regime default, a fixed
3-strategy leaderboard (Carry/Momentum/Mean Reversion with invented Sharpe
ratios), a 4-row enterprise-report list with permanently
Generated/Ready/Queued/Pending metrics, and several 84-88% gauge defaults.
All of these now honestly show "No rows available" / 0% / "UNKNOWN" when
no real data is present instead of a plausible-looking placeholder.
"""
...
def _opportunities(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # No hardcoded fallback: an empty list here honestly renders as "No
    # rows available" instead of a fixed EUR/USD/USD-CHF/AUD-USD sample.
...
def _render_executive(payload: Dict[str, Any]) -> None:
    with panel("Executive AI Command Center", kicker="Executive", meta=payload.get("status", "READY")):
        ...
        with c2:
            render_gauge(extract_score(payload, ["ai_confidence", "confidence", "alpha_score", "composite_score"], 0), "AI Confidence")
...
def _render_regime(payload: Dict[str, Any]) -> None:
    ...
    current = regime.get("regime") or regime.get("market_regime") or regime.get("macro_regime") or "UNKNOWN"
    confidence = safe_float(regime.get("confidence") or regime.get("regime_score") or regime.get("macro_score"), 0.0)
...
def _render_signal_validation(payload: Dict[str, Any]) -> None:
    ...
    success = (validated / total * 100) if total else extract_score(validation, ["success_rate", "validation_rate"], 0)
...
def _render_strategy_lab(payload: Dict[str, Any]) -> None:
    ...
    # No hardcoded fallback leaderboard: an empty list honestly shows "No
    # rows available" instead of fixed Sharpe/win-rate/drawdown numbers for
    # strategies that were never actually backtested here.
    rows = collect_rows(lab, ("strategies", "leaderboard", "results", "rows"))
...
def _render_ai_committee(payload: Dict[str, Any]) -> None:
    ...
    with panel("AI Investment Committee", kicker="Boardroom", meta=str(decision)):
        ...
        with c2:
            render_gauge(extract_score(committee or payload, ["confidence", "consensus", "ai_confidence"], 0), "Committee Consensus")
...
def _render_enterprise_reports(payload: Dict[str, Any]) -> None:
    ...
    # No hardcoded fallback rows or fixed Generated/Ready/Queued/Pending
    # metric labels: both now reflect whatever real report rows/statuses
    # are actually present, and read "No rows available." / "-" otherwise.
    rows = collect_rows(reports, ("reports", "items", "rows"))
    with panel("Enterprise Reporting Center", kicker="Reports"):
        statuses = [str(r.get("Status") or r.get("status") or "-") for r in rows if isinstance(r, dict)]
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Reports", len(rows))
        c2.metric("Generated", sum(1 for s in statuses if s.lower() == "generated"))
        c3.metric("Ready", sum(1 for s in statuses if s.lower() == "ready"))
        c4.metric("Pending", sum(1 for s in statuses if s.lower() in {"pending", "queued"}))
