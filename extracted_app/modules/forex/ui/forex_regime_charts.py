"""
modules/forex/ui/forex_regime_charts.py

Plotly chart helpers for the Regime workspace: regime confidence gauge,
transition-probability bar chart, and regime history timeline. Reads its
data via forex_regime_summary.normalize_regime().

Fixed in this pass (no-fake-data audit): render_transition_probability()
now shows "No rows available." when normalize_regime() reports no real
transition probabilities, instead of silently rendering an empty chart.
This complements the fix in forex_regime_summary.py, which stopped
inventing transition percentages for a fixed "RISK_OFF" regime.
"""
