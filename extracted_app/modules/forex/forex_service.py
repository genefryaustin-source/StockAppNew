...
class ForexService:
    ...
    def get_quote(self, pair: Any = "EUR/USD") -> ForexQuote:
        ...
        Provider chain:
        1. forex_provider_router (cached, cooldown-aware -- checks a
           shared quote cache first, and skips providers that recently
           failed rather than retrying them every call)
        2. forex_quote_aggregator (uncached -- always calls all five
           raw provider modules sequentially with no early exit and no
           cooldown tracking; kept as a fallback for the rare case the
           router itself errors, not as the default path)
        3. deterministic synthetic fallback

        This order was previously reversed (aggregator first, router
        second), meaning every single quote lookup hit the slow,
        uncached path first, always -- confirmed via comments elsewhere
        in this codebase measuring this at 1-9+ seconds per pair, and
        as the dominant cause of multi-second-to-90-second dashboard
        loads and "page appears to hang" reports. Those were fixed at
        specific call sites by routing around this method entirely
        (get_forex_quotes_from_router directly); this fixes the method
        itself, so every caller benefits, including ones not
    ...
    def get_quotes(self, pairs: Optional[List[str]] = None) -> Dict[str, ForexQuote]:
        """
        Fetches all requested pairs in a single parallel, cache-aware
        batch via the provider router, rather than calling get_quote()
        once per pair in a serial loop (the previous implementation --
        for N pairs with no cache warm yet, that meant N sequential
        full provider-chain attempts, each of which could itself take
        several seconds; see get_quote()'s docstring for the measured
        impact of the equivalent per-pair issue).
        """
        ...
        try:
            ...
            raw_by_pair = get_forex_quotes_from_router(selected)

        except Exception:
            raw_by_pair = {}

        result: Dict[str, ForexQuote] = {}
        for pair in selected:
            normalized = normalize_pair(pair)
            raw = raw_by_pair.get(normalized)
            quote = self._coerce_quote(raw, normalized, provider="ProviderRouter") if raw else None
            result[normalized] = quote if quote is not None else self.get_quote(normalized)
