...
def render_forex_governor_dashboard(  db=None,
    user=None,) -> None:
    st.title("Forex Resource Governor Dashboard")
    if st.button("Evaluate Limits", key="fx_governor_eval"):
        st.json(ForexResourceGovernor().evaluate_limits())
    _render_snapshot("Governor Snapshot")
