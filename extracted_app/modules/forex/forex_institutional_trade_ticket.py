...
class ForexInstitutionalTradeTicket:
    ...
    def quote_ticket(
        ...
        *,
        ...
        has_account_data = bool(account_snapshot)
        equity = _safe_float(account_snapshot.get("equity") or account_snapshot.get("cash_balance"), 0.0)
        ...
        if entry_price in (None, ""):
            live_price = self._last_price(pair_norm)
        no_price_available = entry_price in (None, "") and live_price is None
        price = _safe_float(entry_price, live_price if live_price is not None else 0.0)
        ...
        warnings = []
        if no_price_available:
            warnings.append("No entry price provided and no live quote available for this pair; risk/margin figures below are not meaningful.")
        if stop is None:
            warnings.append("No stop loss provided.")
        if target is None:
            warnings.append("No take profit provided.")
        if not has_account_data:
            warnings.append("No live account data available; margin/equity figures below default to $0 rather than an assumed balance.")
    ...
    def _default_account_snapshot(self) -> Dict[str, Any]:
        """
        Real account snapshot via forex_portfolio_engine. This previously
        referenced self.tenant_id/self.user_id/self.portfolio_id, which
        __init__ never set -- every call raised AttributeError and
        silently fell through to a hardcoded {"equity": 100000.0,
        "margin_available": 100000.0, "leverage": 50.0} placeholder
        account, regardless of what the real portfolio held. That bug is
        now fixed; on genuine failure this returns an empty dict instead
        of a fabricated starter balance, and callers treat missing
        equity/margin as "unknown" rather than $100k.
        """
        try:
            ...
            data = account.to_dict() if hasattr(account, "to_dict") else {}
            return data if isinstance(data, dict) else {}
        except Exception:
            return {}

    def _last_price(self, pair: str) -> Optional[float]:
        """
        Real live quote (mid/last) from the price service. Returns None
        when no live quote is available -- callers must warn rather than
        silently substitute a fabricated per-pair placeholder price.
        """
        try:
