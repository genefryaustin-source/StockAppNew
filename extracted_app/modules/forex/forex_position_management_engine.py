...
class ForexPositionManagementEngine:
    ...
    def _build_context(
        ...
        # execution_id is left at its auto-generated value (not
        # overwritten from the position dict) -- a position can span
        # multiple fills/orders over its life, so there's no single
        # "the" execution_id to attribute it to. A fresh one
        # identifies this specific modify/close/scale operation.
        ...
        # "units", not "quantity" -- ExecutionPositionRepository._from_row()
        # never produces a "quantity" key, so this used to always
        # default to 0 regardless of the position's real size.
        ...
        # "avg_entry_price", not "avg_price" -- that key doesn't exist
        # on these rows either.
        avg_price = position.get("avg_entry_price")

        context.average_fill_price = avg_price

        context.requested_price = avg_price
        ...
        context.leverage = position.get(
            "leverage",
        )

        # broker/broker_order_id are order-level concepts; a position
        # can result from multiple orders (scale-ins), so there's no
        # single broker_order_id to attribute to it. Left at
        # ExecutionContext's own defaults ("paper" / None) rather than
        # overwritten with position.get(...) calls that always
        # returned None anyway (positions don't carry these keys).
