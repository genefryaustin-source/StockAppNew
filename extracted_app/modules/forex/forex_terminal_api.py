...
class ForexTerminalAPI:
    ...
    def get_terminal_snapshot(self, **kwargs) -> Dict[str, Any]:
        try:
            ...
            # persist=True previously fired unconditionally on every single
            # call, regardless of the caller's refresh throttle -- each
            # persist does a real INSERT + COMMIT into
            # forex_portfolio_snapshots, which real console timing showed
            # taking multiple seconds (observed as the dominant remaining
            # cost inside get_snapshot() after the liquidity-score cache
            # fix brought calculate_risk() down to ~2ms). Persisting on
            # every Streamlit rerun (i.e. every click) is unnecessary --
            # tie persistence to the same refresh decision the caller
            # already made (ForexTerminalDashboard.render() throttles
            # "refresh" to at most once every 15s) so the equity-curve
            # history table gets one row per real refresh instead of one
            # per click.
            snapshot = engine.get_terminal_snapshot(
                account_id=kwargs.get("account_id"),
                portfolio_id=kwargs.get("portfolio_id"),
                refresh=kwargs.get("refresh", True),
                persist=kwargs.get("persist", kwargs.get("refresh", True)),
                include_orders=True,
    ...
    def refresh_terminal(self, **kwargs) -> Dict[str, Any]:
        try:
        ...
        # Previously called self.get_terminal_snapshot(refresh=True) with no
        # tenant_id/user_id/portfolio_id, which silently resolved to the
        # default (portfolio_id IS NULL) account instead of whichever
        # portfolio the user has active -- so hitting the terminal's manual
        # refresh button would reset the view back to an empty portfolio.
        # Forward the caller's scoping kwargs through so refresh keeps the
        # same portfolio in view.
        snapshot_kwargs = dict(kwargs)
        snapshot_kwargs["refresh"] = True
        return {"status": "REFRESHED", "refresh": refresh, "snapshot": self.get_terminal_snapshot(**snapshot_kwargs)}
