...
class ForexAlphaModel:
    ...
    def __init__(
        ...
        # Sprint 28: run_alpha_model() is invoked independently by ~6
        # different consumers per page render (institutional scanner,
        # strategy lab, sentiment engine, portfolio optimizer, AI trade
        # assistant, AI command center) with identical inputs every time --
        # none of them share a result. A short TTL cache keyed on the
        # actual call inputs lets every consumer within the same render
        # reuse one computation instead of repeating the full 28-pair scan.
        self._run_cache: Dict[Tuple[Any, ...], Tuple[float, Dict[str, Any]]] = {}
        self._run_cache_lock = threading.Lock()
        self._run_cache_ttl_seconds = 8.0
    ...
    def run_alpha_model(
        ...
        cache_key = None
        if not force_refresh and quotes is None:
            cache_key = (tuple(pairs), round(account_size, 2), round(risk_per_trade_pct, 6))
            with self._run_cache_lock:
                cached = self._run_cache.get(cache_key)
            if cached is not None:
                cached_at, cached_payload = cached
                if (time.perf_counter() - cached_at) <= self._run_cache_ttl_seconds:
                    _alpha_debug(
                        f"{run_id} | RUN CACHE HIT",
                        model_id=id(self),
                        cache_key=cache_key,
                        age_ms=round((time.perf_counter() - cached_at) * 1000, 2),
                    )
        ...
        try:
            ...
            if cache_key is not None:
                with self._run_cache_lock:
                    self._run_cache[cache_key] = (time.perf_counter(), payload)
