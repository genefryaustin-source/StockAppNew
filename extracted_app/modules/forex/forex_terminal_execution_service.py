...
def get_forex_terminal_execution_service(db: Optional[Any] = None) -> ForexTerminalExecutionService:
    """
    Always constructs a fresh instance -- this used to cache a
    module-level singleton and only replace it when the cached
    instance's db was None, which meant in a multi-request server
    context, whichever request happened to construct this first would
    have every subsequent call (including from other tenants/sessions)
    silently reuse that same, possibly stale db session. Construction
    here is cheap (attribute assignment only; execution_service/
    execution_pipeline are lazily built on first use), so there's no
    performance reason to share an instance across calls.
    """
    return ForexTerminalExecutionService(db=db)