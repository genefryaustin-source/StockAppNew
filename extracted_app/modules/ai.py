...
def ai_generate_memo(symbol, profile, fin_health, valuation, peer_table, tech, risks, summary_text) -> str:
    ...
    def ai_enabled() -> bool:
        ...
        return bool(get_provider_key("OPENAI_API_KEY"))

    def _client():
        from openai import OpenAI
        from modules.admin.tenant_api_keys import get_provider_key
        # Explicitly pass the resolved key -- OpenAI() with no api_key= only
        # ever reads os.environ, which can't see a per-tenant DB-stored key.
        return OpenAI(api_key=get_provider_key("OPENAI_API_KEY"))
